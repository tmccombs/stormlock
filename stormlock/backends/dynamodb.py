...
class DynamoDB(Backend):
    def __init__(
        ...
        self._ConditionFailedException = (
    ...
    def lock(
        ...
        condition = (Attr('resource').not_exists()
                     or Attr('expires').le(item['created']))
        try:
    ...
    def renew(self, resource: str, lease_id: str, ttl: timedelta):
        ...
        condition = (
                Attr('lease').eq(lease_id)
                and Attr('expires').gt(int(now.timestamp()))
                )
        expire = now + ttl
